...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _firstNameController = TextEditingController(text: viewModel.user?.firstName ?? '');
      _lastNameController = TextEditingController(text: viewModel.user?.lastName ?? '');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (profileViewModel.errorMessage != null) {
      return Scaffold(
        body: Center(child: Text(profileViewModel.errorMessage!)),
      );
    }
